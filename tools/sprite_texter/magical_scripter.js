...
function readOriginalScript(e) {
  ...
  fileReader.onload = function (e) {
    ...
    yaml_script = [];
    yaml_script.length = HEADER_LENGTH;
    //if (yaml_script) {
      enable();
    //}
  };
...
function generateJapaneseScript() {
  var control_characters = {
    ...
    0xFA:function(state) { state.idx++; }, // TODO: Unknown control character
    0xFE:function(state) { state.idx++; }, // TODO: Unknown control character
    0xFF:function(state) { state.idx++; } // TODO: Unknown control character
  };

  var character_count = 0;
  var translated_character_count = 0;
  for (var ii = 0; ii < HEADER_LENGTH/2 - 1; ii++) {
    var state = {
      italics: false,
      color: '',
      idx: header[ii],
...
function generateOriginalHeader() {
  ...
  for (var ii = 0; ii < HEADER_LENGTH; ii += 2) {
    ...
    offset = newoffset;
    header.push(offset + count*65536 + HEADER_LENGTH);
  }
}
